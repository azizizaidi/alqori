...
@section('title', __('test.create'))

@section('content')
<div class="row justify-content-center">
    <div class="col-md-6">
        <div class="card">
            <div class="card-header">{{ __('test.create') }}</div>
            <form method="POST" action="{{ route('tests.store') }}" accept-charset="UTF-8">
                ...
                <div class="card-footer">
                    <input type="submit" value="{{ __('test.create') }}" class="btn btn-success">
                    <a href="{{ route('tests.index') }}" class="btn btn-link">{{ __('app.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
